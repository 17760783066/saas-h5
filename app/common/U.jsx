...
var U = (function () {
    ...
    var date = (function () {
        ...
        let duration2Str = (duration, options) => {
            let units = {
                'I': '分钟',
                'H': '小时',
                'D': '天',
            ...
            if (str.isEmpty(duration) || duration.length < 2) {
                return options && options.defaultStr || '--';
            } else {
                let length = duration.length;
                let unit = duration.substring(length - 1);
                let num = parseInt(duration.substring(0, length - 1));
                if (duration === '1S') {
                    return '无';
                } else if (num === 0) {
                    return '永久';
        ...
        return {
            ...
            splashTime, seconds2DH, seconds2DHMS, daysDiff, duration2Str
        };
    })();
    ...
    let base64 = (() => {
        ...
        let decode = (str) => {
            return AwesomeBase64.decodeString(str);
        };
        ...
        return { getBlobBydataURI, textToBase64QrCode, decode };
    })();
    ...
    let getIdFromUrl = () => {
        let id = U.getParameter('id');
        if (U.str.isEmpty(id)) {
            Toast.show({  icon: 'fail',content: '参数错误' });
            return;
        }
        return U.base64.decode(id.toString());
    };
